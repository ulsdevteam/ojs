...
/**
 * @file plugins/generic/sushiLite/classes/reports/JR1_r4_1.inc.php
 *
 ...
 * 
 * @brief Journal Report 1, release 4.1
 */

import('plugins.reports.counter.classes.releases.CounterRelease4_1');

class JR1_r4_1 extends CounterRelease4_1 {
	
	/**
	 * Constructor
	 */
	function JR1_r4_1($parentPluginCategory, $parentPluginName) {
			...
			foreach ($results as $rs) {
				...
				if ($lastJournal != $rs[STATISTICS_DIMENSION_CONTEXT_ID]) {
					if ($lastJournal != 0 && $metrics) {
						$reportItems[] = $this->_createReportItem($lastJournal, $metrics);
						$metrics = array();
					}
				}
